...
import { useRouterType } from '@module/contexts/router-picker'
import { Action, IResourceItem } from '@module/interfaces'
import { composeRoute, getActionRoutesFromResource } from '@module/definitions'
import { useResource } from '@module/hooks/resource'
...
export const useGetToPath = (): GetToPathFn => {
  ...
  const fn = React.useCallback(
    ({ resource, action, meta }: UseToPathParams): string | undefined => {
      ...
      const actionRoute = actionRoutes.find((item) => item.action === action)
        ?.route
