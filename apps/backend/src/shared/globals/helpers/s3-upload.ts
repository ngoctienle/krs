...
class S3Upload {
  bucket: string = 'krs-storage'
	...
	public async copy(slug: string, new_slug: string, type: string): Promise<void> {
		const objectCoppy = {
			Bucket: this.bucket,
		...
		return new Promise<void>((resolve, reject) => {
			this.s3Client.send(coppyCommand, (err, data) => {
				if (err) {
					...
					log.info(`Successfully uploaded data to ${objectCoppy.Bucket}/${objectCoppy.Key}`)
	...
	public async move(slug: string, new_slug: string, type: string): Promise<void> {
		if(slug === new_slug) {
			return
		}
		...
		return new Promise<void>((resolve, reject) => {
			this.s3Client.send(deleteCommand, (err, data) => {
				if (err) {
					...
					log.info(`Successfully uploaded data to ${objectDelete.Bucket}/${objectDelete.Key}`)
